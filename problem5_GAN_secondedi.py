#第五题的第二个版本
#使用了Wgan的方法，将判别器的损失函数改为了Wasserstein距离，还有权重剪切
import torch
...
ngf = 28 #生成器的特征图大小
ndf = 28 #判别器的特征图大小
num_epochs = 151 #训练的轮数
lr = 0.0002 #学习率
beta1 = 0.9 #Adam优化器的参数,改成0.9试试
ngpu = 1 #GPU的数量
...
class Discriminator(nn.Module):
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv2d(1, 32, 3, 2, 1, bias=False),  # 输出尺寸 (64) x 14 x 14
            nn.BatchNorm1d(32),
            nn.LeakyReLU(0.2, inplace=True),
            
            # 输出尺寸 (64) x 14 x 14
            nn.Conv2d(32, 64, 3, 2, 1, bias=False),  # 输出尺寸 (128) x 7 x 7
            nn.BatchNorm1d(64),
            nn.LeakyReLU(0.2, inplace=True),
            
            # 输出尺寸 (128) x 7 x 7
            nn.Conv2d(64, 128, 3, 2, 1, bias=False),  # 输出尺寸 (256) x 4 x 4
            nn.BatchNorm1d(128),
            nn.LeakyReLU(0.2, inplace=True),
            
            # 输出尺寸 (256) x 4 x 4
            nn.Conv2d(128, 1, 4, 1, 0, bias=False),  # 输出尺寸 (1) x 1 x 1
